...
final class ArchieML {
	...
	public function parse( $input ) {
		...
		while ( $input ) {
			...
			} elseif ( !$this->isSkipping && preg_match( self::startKey, $input, $matches ) &&
				( !$this->stackScope || $this->stackScope['arrayType'] !== 'simple' ) ) {

				$this->parseStartKey( $matches[1], $matches[2] );

			} elseif ( !$this->isSkipping && preg_match( self::arrayElement, $input, $matches ) && $this->stackScope && $this->stackScope['array'] &&
			...
			} elseif ( !$this->isSkipping && preg_match( self::scopePattern, $input, $matches ) ) {

				$this->parseScope( $matches[1], $matches[2], $matches[3] );

			} elseif ( preg_match( self::nextLine, $input, $matches ) ) {
